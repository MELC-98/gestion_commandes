...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($erreurs)) {
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM clients WHERE code_client = ?");
    ...
    if (empty($erreurs)) {
        try {
            $stmt = $pdo->prepare("INSERT INTO clients (code_client, nom, prenom, email, telephone) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$code_client, $nom, $prenom, $email, $telephone ?: null]);
            $succes = true;
...
<head>
    ...
    <title>Ajouter un Client - Gestion des Commandes</title>
            ...
            <div class="form-container">
                <form method="POST" action="">
                    <div class="form-group">
                        ...
                        <input type="text" id="code_client" name="code_client" value="<?php echo htmlspecialchars($_POST['code_client'] ?? ''); ?>" required>
                    </div>
